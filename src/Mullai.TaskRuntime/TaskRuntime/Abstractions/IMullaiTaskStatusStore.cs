...
public interface IMullaiTaskStatusStore
{
    ...
    Task MarkRunningAsync(MullaiTaskWorkItem workItem, string? response = null, MullaiUsage? usage = null,
        CancellationToken cancellationToken = default);

    Task MarkRetryScheduledAsync(MullaiTaskWorkItem workItem, string error,
        CancellationToken cancellationToken = default);

    Task MarkSucceededAsync(MullaiTaskWorkItem workItem, string response, MullaiUsage? usage = null,
        CancellationToken cancellationToken = default);
    ...
    Task<IReadOnlyCollection<MullaiTaskStatusSnapshot>> GetRecentAsync(int take = 50,
        CancellationToken cancellationToken = default);

    Task<MullaiUsage> GetTotalUsageAsync(string sessionKey, CancellationToken cancellationToken = default);
}