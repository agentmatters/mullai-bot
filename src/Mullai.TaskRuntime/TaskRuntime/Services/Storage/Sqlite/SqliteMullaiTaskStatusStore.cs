...
public sealed class SqliteMullaiTaskStatusStore : IMullaiTaskStatusStore
{
        ...
        IWorkflowRunEventStore runEventStore)
    {
        ...
        while (reader.Read())
            if (reader.GetString(1) == "input_tokens")
                hasInputTokens = true;
        if (!hasInputTokens)
        {
    ...
    {
        return UpsertAsync(workItem, MullaiTaskState.Queued, null, null, null, cancellationToken);
    }

    public Task MarkRunningAsync(MullaiTaskWorkItem workItem, string? response = null, MullaiUsage? usage = null,
        CancellationToken cancellationToken = default)
    {
        return UpsertAsync(workItem, MullaiTaskState.Running, response, null, usage, cancellationToken);
    }

    public Task MarkRetryScheduledAsync(MullaiTaskWorkItem workItem, string error,
        CancellationToken cancellationToken = default)
    {
        return UpsertAsync(workItem, MullaiTaskState.RetryScheduled, null, error, null, cancellationToken);
    }

    public Task MarkSucceededAsync(MullaiTaskWorkItem workItem, string response, MullaiUsage? usage = null,
        CancellationToken cancellationToken = default)
    {
        return UpsertAsync(workItem, MullaiTaskState.Succeeded, response, null, usage, cancellationToken);
    }

    public Task MarkFailedAsync(MullaiTaskWorkItem workItem, string error,
        CancellationToken cancellationToken = default)
    {
        return UpsertAsync(workItem, MullaiTaskState.Failed, null, error, null, cancellationToken);
    }
    ...
    public Task<MullaiTaskStatusSnapshot?> GetAsync(string taskId, CancellationToken cancellationToken = default)
    {
        ...
        if (!reader.Read()) return Task.FromResult<MullaiTaskStatusSnapshot?>(null);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        while (reader.Read()) results.Add(ReadSnapshot(reader));
    ...
    public Task<MullaiUsage> GetTotalUsageAsync(string sessionKey, CancellationToken cancellationToken = default)
    {
        ...
        using var command = connection.CreateCommand();
        command.CommandText =
            "SELECT SUM(input_tokens), SUM(output_tokens), SUM(total_tokens) FROM task_status WHERE session_key = $sessionKey";
        command.Parameters.AddWithValue("$sessionKey", sessionKey);
        using var reader = command.ExecuteReader();
        if (!reader.Read() || reader.IsDBNull(0)) return Task.FromResult(new MullaiUsage(0, 0, 0));
    ...
    private async Task PruneRunsAsync(string workflowId, CancellationToken cancellationToken)
    {
        ...
        while (reader.Read()) taskIds.Add(reader.GetString(reader.GetOrdinal("task_id")));
        ...
        for (var i = 0; i < taskIds.Count; i++) deleteCommand.Parameters.AddWithValue(parameters[i], taskIds[i]);
