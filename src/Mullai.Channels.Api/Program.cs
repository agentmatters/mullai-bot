...
app.MapPost("/api/webhooks/{channelId}",
        async (string channelId, [FromBody] object payload, ChannelManager channelManager) =>
        {
            var adapter = channelManager.GetAdapter(channelId);
            if (adapter == null) return Results.NotFound($"Channel adapter not found for '{channelId}'");
